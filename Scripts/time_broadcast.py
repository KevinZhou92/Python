#!/usr/bin/python3
import pyttsx
import os

def num2voice(number):
	number = int(number)
	number_decimal = number // 10
	number_base = number % 10
	NUMBER_CONSTANT = {0:"zero ", 1:"one", 2:"two", 3:"three", 4:"four", 5:"five", 6:"six", 7:"seven",  
                8:"eight", 9:"nine", 10:"ten", 11:"eleven", 12:"twelve", 13:"thirteen",  
                14:"fourteen", 15:"fifteen", 16:"sixteen", 17:"seventeen", 18:"eighteen", 19:"nineteen" };  
	IN_HUNDRED_CONSTANT = {2:"twenty", 3:"thirty", 4:"forty", 5:"fifty", 6:"sixty", 7:"seventy", 8:"eighty", 9:"ninety"} 
	
	if number_decimal == 0:
		return NUMBER_CONSTANT[number_base]
	else:
		if number > 20 :
			return IN_HUNDRED_CONSTANT[number_decimal] + ' '+  NUMBER_CONSTANT[number_base]
		elif number == 20 :
			return IN_HUNDRED_CONSTANT[2]
		else :
			return NUMBER_CONSTANT[number]

time = os.popen('date +%R').readline().strip("\n").split(":")
hour = num2voice(time[0])
minute = num2voice(time[1])


engine = pyttsx.init()
rate = engine.getProperty('rate')
engine.setProperty('rate', rate - 50)
speech = 'My master. The current time is ' + hour + ' ' + minute
print(speech)
engine.say(speech)
engine.say('hi')
engine.runAndWait()

